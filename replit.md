...
- **JSON Product Editor:** Pure frontend tool for editing product catalog JSON files (no backend). Supports drag & drop / file picker for multiple .json files, category tabs, product navigation with search/filter. **Fully dynamic rendering**: no hardcoded field interface — all product keys are auto-detected and rendered based on value type (string→text input, long string→textarea, number→number input, boolean→toggle, string array→tag input, object array→item editor, nested object→collapsible sub-section with recursive rendering). Every object section has "Add Field" button (with key name + type selector: Text/Long Text/Number/Boolean/Array/Object) and every field has a "Delete" button with confirmation. Special handling: price field converts kuruş↔TL, image_url shows preview, template.sub_type uses dropdown from metadata.sub_types, FAQ arrays get Q&A editor. Metadata section also fully dynamic. Supports any JSON structure (e.g. cleaned_content with nested url/text pairs). Download current/all as formatted JSON. Frontend at `/json-editor`.
