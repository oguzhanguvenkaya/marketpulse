...
- **JSON Product Editor:** Pure frontend tool for editing product catalog JSON files (no backend). Supports drag & drop / file picker for multiple .json files, category tabs, product navigation with search/filter, dynamic form generation adapting to each category's unique template.fields structure (auto-detects types: string→text, number→number, boolean→toggle, array→comma-separated, object→nested fields). Sections: Basic Info (SKU, price in TL/kuruş, image preview, categories), Template (group, sub_type dropdown, dynamic fields), Content (6 textarea fields), Relations (5 tag-style inputs), FAQ (add/remove Q&A pairs), Metadata (editable group info, sub_types, template_fields reference). Download current/all as formatted JSON. Frontend at `/json-editor`.
