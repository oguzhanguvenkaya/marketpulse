  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm mb-4">
              ...
              {product.seller_name && (
                <p className="text-neutral-400">
                  Seller: <span className="text-neutral-200">{product.seller_name}</span>
                  {product.seller_rating && (
                    <span className="ml-2 text-warning">({product.seller_rating.toFixed(1)})</span>
                  )}
                </p>
              )}
              {product.category_hierarchy && <p className="text-neutral-400">Category: <span className="text-neutral-200">{product.category_hierarchy}</span></p>}
              {product.origin_country && <p className="text-neutral-400">Origin: <span className="text-neutral-200">{product.origin_country}</span></p>}
              {product.sku && <p className="text-neutral-400">SKU: <span className="text-neutral-200">{product.sku}</span></p>}
            ...
            {(product.coupons && product.coupons.length > 0) && (
              <div className="mb-3">
                <h3 className="text-sm font-medium text-neutral-400 mb-2">Coupons</h3>
            ...
            {(product.campaigns && product.campaigns.length > 0) && (
              <div className="mb-4">
                <h3 className="text-sm font-medium text-neutral-400 mb-2">Campaigns</h3>
                <div className="flex flex-wrap gap-2">
      ...
      <div className="card-dark overflow-hidden">
        ...
        <div className="p-6">
          {activeTab === 'info' && (
            <div>
              {product.description ? (
                <p className="text-neutral-300 whitespace-pre-wrap leading-relaxed">{product.description}</p>
              ) : (
                  ...
                  <table className="table-dark">
                    ...
                    <tbody>
                      {product.other_sellers.map((seller, i) => (
                        <tr key={i}>
                          <td className="text-neutral-200">{seller.seller_name}</td>
                ...
                <div className="space-y-4">
                  {product.reviews.map((review, i) => (
                    <div key={i} className="border-b border-white/5 pb-4 last:border-b-0">
                      ...
                      {review.review_text && (
                        <p className="text-neutral-400 text-sm">{review.review_text}</p>
                      )}
                      {review.seller_name && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="card-dark p-5">
            ...
            <Plot
              ...
              layout={{
                ...plotLayout,
                yaxis: { ...plotLayout.yaxis, title: { text: 'Price (TL)', font: { color: '#808080' } } }
              }}
          ...
          <div className="card-dark p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-white">Rating History</h2>
            </div>
            <Plot
              data={[ratingData]}
              layout={{
                ...plotLayout,
                yaxis: { ...plotLayout.yaxis, title: { text: 'Rating', font: { color: '#808080' } }, range: [0, 5] }
              }}
              config={{ responsive: true, displayModeBar: false }}
              style={{ width: '100%' }}
            />
      ...
      <div className="card-dark p-5">
        ...
        {analysis && (
          <div className="bg-dark-700/50 rounded-lg p-4 text-neutral-300 whitespace-pre-wrap border border-white/5">
            {analysis}
          </div>
        )}
      </div>
