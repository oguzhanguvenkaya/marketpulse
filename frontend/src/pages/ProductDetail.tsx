  ...
  if (loading) {
    return (
      <div className="card-dark p-12 text-center">
        ...
        <p className="mt-4 text-[#9e8b66] dark:text-[#6B8F80]">Loading product...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="card-dark p-12 text-center">
        ...
        <p className="text-[#9e8b66] dark:text-[#6B8F80]">Product not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 md:space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm text-neutral-500">
        ...
        <span className="text-[#5f471d] dark:text-[#A7C4B8]">Detail</span>
      </div>

      <div className="card-dark p-4 md:p-6">
        <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
          {product.image_url ? (
            <img src={product.image_url} alt="" className="w-full sm:w-56 md:w-64 lg:w-48 h-56 md:h-64 lg:h-48 object-cover rounded-lg border border-[#5b4824]/12 dark:border-[#4ADE80]/12" />
          ) : (
            <div className="w-full sm:w-56 md:w-64 lg:w-48 h-56 md:h-64 lg:h-48 rounded-lg bg-[#f0e8d8] dark:bg-[#1C2E28] flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-xl md:text-2xl font-bold text-[#0f1419] dark:text-[#F0FDF4] mb-3 break-words">{product.name}</h1>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-xs md:text-sm mb-4">
              <p className="text-[#9e8b66] dark:text-[#6B8F80]">Platform: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.platform}</span></p>
              {product.seller_name && (
                <p className="text-[#9e8b66] dark:text-[#6B8F80]">
                  Seller: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.seller_name}</span>
                  {product.seller_rating && (
                    <span className="ml-2 text-warning">({product.seller_rating.toFixed(1)})</span>
                  )}
                </p>
              )}
              {product.category_hierarchy && <p className="text-[#9e8b66] dark:text-[#6B8F80]">Category: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.category_hierarchy}</span></p>}
              {product.origin_country && <p className="text-[#9e8b66] dark:text-[#6B8F80]">Origin: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.origin_country}</span></p>}
              {product.sku && <p className="text-[#9e8b66] dark:text-[#6B8F80]">SKU: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.sku}</span></p>}
              {product.barcode && <p className="text-[#9e8b66] dark:text-[#6B8F80]">Barcode: <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.barcode}</span></p>}
            </div>
            ...
            {(product.coupons && product.coupons.length > 0) && (
              <div className="mb-3">
                <h3 className="text-sm font-medium text-[#9e8b66] dark:text-[#6B8F80] mb-2">Coupons</h3>
            ...
            {(product.campaigns && product.campaigns.length > 0) && (
              <div className="mb-4">
                <h3 className="text-sm font-medium text-[#9e8b66] dark:text-[#6B8F80] mb-2">Campaigns</h3>
                <div className="flex flex-wrap gap-2">
      ...
      <div className="card-dark overflow-hidden">
          ...
          <nav className="flex overflow-x-auto scrollbar-thin">
            <button
              ...
              className={`px-4 md:px-6 py-3.5 md:py-4 border-b-2 font-medium text-xs md:text-sm transition-all whitespace-nowrap ${
                activeTab === 'info' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-[#9e8b66] dark:text-[#6B8F80] hover:text-[#3d3427] dark:hover:text-[#F0FDF4]'
              }`}
            >
              Product Info
            </button>
            <button
              ...
              className={`px-4 md:px-6 py-3.5 md:py-4 border-b-2 font-medium text-xs md:text-sm transition-all whitespace-nowrap ${
                activeTab === 'sellers' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-[#9e8b66] dark:text-[#6B8F80] hover:text-[#3d3427] dark:hover:text-[#F0FDF4]'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 md:px-6 py-3.5 md:py-4 border-b-2 font-medium text-xs md:text-sm transition-all whitespace-nowrap ${
                activeTab === 'reviews' ? 'border-accent-primary text-accent-primary' : 'border-transparent text-[#9e8b66] dark:text-[#6B8F80] hover:text-[#3d3427] dark:hover:text-[#F0FDF4]'
              }`}
            >
        ...
        <div className="p-4 md:p-6">
          {activeTab === 'info' && (
            <div>
              {product.description ? (
                <p className="text-[#5f471d] dark:text-[#A7C4B8] whitespace-pre-wrap leading-relaxed">{product.description}</p>
              ) : (
                  ...
                  <table className="table-dark">
                    ...
                    <tbody>
                      {product.other_sellers.map((seller, i) => (
                        <tr key={i}>
                          <td className="text-[#3d3427] dark:text-[#A7C4B8]">{seller.seller_name}</td>
              ...
              {product.reviews && product.reviews.length > 0 ? (
                <div className="space-y-4">
                  {product.reviews.map((review, i) => (
                    <div key={i} className="border-b border-[#5b4824]/8 dark:border-[#4ADE80]/8 pb-4 last:border-b-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                        <div className="flex items-center gap-3">
                          <span className="font-medium text-[#3d3427] dark:text-[#A7C4B8]">{review.author || 'Anonymous'}</span>
                          {review.rating && (
                            <span className="text-warning text-sm">
                              {'★'.repeat(review.rating)}
                              <span className="text-[#b5a382] dark:text-[#6B8F80]">{'★'.repeat(5 - review.rating)}</span>
                            </span>
                          )}
                        </div>
                        {review.review_date && (
                      ...
                      {review.review_text && (
                        <p className="text-[#9e8b66] dark:text-[#6B8F80] text-sm">{review.review_text}</p>
                      )}
                      {review.seller_name && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          <div className="card-dark p-4 md:p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0f1419] dark:text-[#F0FDF4]">Price History</h2>
            </div>
            <Plot
              data={[priceData]}
              layout={{
          ...
          <div className="card-dark p-4 md:p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-lg font-semibold text-[#0f1419] dark:text-[#F0FDF4]">Rating History</h2>
            </div>
            <Plot
              data={[ratingData]}
              layout={{
      ...
      <div className="card-dark p-4 md:p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-[#0f1419] dark:text-[#F0FDF4]">AI Analysis</h2>
        </div>
        ...
        {analysis && (
          <div className="bg-[#f7eede]/50 dark:bg-[#162420]/50 rounded-lg p-4 text-[#5f471d] dark:text-[#A7C4B8] whitespace-pre-wrap border border-[#5b4824]/8 dark:border-[#4ADE80]/8">
            {analysis}
          </div>
        )}
      </div>
