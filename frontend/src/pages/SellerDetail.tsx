    ...
    <div className="space-y-5 md:space-y-6">
      <div className="flex items-start gap-3 md:gap-4">
        <button
          ...
          className="p-2 rounded-lg bg-[#f7eede] hover:bg-[#f0e8d8] transition-colors shrink-0"
        >
        ...
        <div className="min-w-0">
          <h1 className="text-xl md:text-2xl font-bold text-[#0f1419] truncate">{merchantName || 'Seller'}</h1>
          <p className="text-sm md:text-base text-[#9e8b66] mt-1">
      ...
      <div className="card-dark p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 md:gap-4 mb-4">
          ...
          <div className="relative">
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-48 max-w-[calc(100vw-2rem)] bg-[#f7eede] rounded-lg shadow-lg border border-dark-500 z-10">
                <button
                  onClick={() => handleExport('all')}
                  className="w-full px-4 py-2 text-left text-[#5f471d] hover:bg-[#f0e8d8] rounded-t-lg"
                >
                  Export All
                </button>
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-[#5f471d] hover:bg-[#f0e8d8] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                ...
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-[#5f471d] hover:bg-[#f0e8d8] rounded-b-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-3 text-[#9e8b66]">Loading products...</span>
          </div>
            ...
            <table className="table-dark w-full min-w-[980px]">
              <thead>
                <tr>
                  ...
                  <th className="text-center border-l border-[#e5e0d2]" colSpan={3}>
                ...
                <tr>
                  <th className="text-right border-l border-[#e5e0d2] text-xs font-normal text-neutral-500">Threshold</th>
              ...
              <tbody>
                {filteredProducts.map((product) => {
                  ...
                  <tr 
                    ...
                    className={`${product.price_alert || product.campaign_alert ? (product.price_alert ? 'bg-red-50' : 'bg-orange-50') : ''} hover:bg-[#f7eede] transition-colors`}
                  >
                    <td>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="max-w-[250px]">
                          <div className="text-[#0f1419] font-medium truncate" title={product.product_name}>
                    ...
                    <td>
                      <div className="text-[#5f471d] text-sm">{product.sku || '-'}</div>
                      <div className="text-neutral-500 text-xs">{product.barcode || '-'}</div>
                    </td>
                    <td className="text-[#5f471d]">{product.brand || '-'}</td>
                    
                    {/* Price Alert columns */}
                    <td className="text-right text-[#9e8b66] border-l border-[#e5e0d2]">
                      {formatPrice(product.threshold_price)}
                    </td>
                    <td className="text-right">
                      <span className={`font-semibold ${product.price_alert ? 'text-danger' : 'text-[#0f1419]'}`}>
                    ...
                    <td className="text-right">
                      {product.campaign_price ? (
                        <>
                          <span className={`font-semibold ${product.campaign_alert ? 'text-warning' : 'text-[#0f1419]'}`}>
