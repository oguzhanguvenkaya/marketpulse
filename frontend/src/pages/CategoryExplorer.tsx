  ...
  return (
    <div className="flex gap-6 pb-10 min-h-[calc(100vh-80px)]">
      <aside className="hidden lg:block w-64 flex-shrink-0">
        <div className="sticky top-4 rounded-xl border border-accent-primary/12 p-4 overflow-hidden bg-gradient-to-b from-surface-card to-dark-900">
          <CategoryFilters {...ce} />
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0 space-y-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowMobileFilters(true)}
              className="lg:hidden px-3 py-2 text-sm rounded-lg border border-accent-primary/12 text-text-body hover:bg-accent-primary/5">
            ...
            {viewMode === 'category_page' && (
              <button
                ...
                className={`px-3 py-2 text-sm rounded-lg border transition-colors flex items-center gap-1.5 ${
                  showDetailPanel ? 'border-text-muted/20 bg-text-muted/8 text-text-muted' : 'border-accent-primary/12 text-text-body hover:bg-accent-primary/5'
                }`}
              >
            ...
            <button
              ...
              className={`px-3 py-2 text-sm rounded-lg border transition-colors flex items-center gap-1.5 ${
                showScraper ? 'border-accent-primary/20 bg-accent-primary/8 text-accent-primary' : 'border-accent-primary/12 text-text-body hover:bg-accent-primary/5'
              }`}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl bg-accent-primary/5 w-fit">
          <button
            onClick={() => setViewMode('my_products')}
            className={`px-4 py-2 text-sm rounded-lg font-medium transition-colors ${
              viewMode === 'my_products' ? 'bg-accent-primary/8 text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 text-sm rounded-lg font-medium transition-colors ${
              viewMode === 'category_page' ? 'bg-accent-primary/8 text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'
            }`}
          >
            <span className="flex items-center gap-1.5">
            ...
            {breadcrumbParts.map((bc, i) => (
              <span key={i} className="flex items-center gap-1.5">
                ...
                <button onClick={() => selectCategory(bc.path)}
                  className={`hover:text-accent-primary transition-colors ${i === breadcrumbParts.length - 1 ? 'text-accent-primary font-medium' : 'text-text-muted'}`}>
                  {bc.name}
                </button>
              </span>
            ))}
            <button onClick={() => { setSelectedCategory(''); setExpandedCategories(new Set()); }} className="ml-2 text-text-faded hover:text-red-600 transition-colors">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="relative flex-1 w-full sm:w-auto">
            ...
            <input
              ...
              className="w-full bg-dark-800 border border-accent-primary/12 rounded-lg pl-10 pr-4 py-2.5 text-sm text-text-secondary placeholder:text-text-faded focus:outline-none focus:border-accent-primary/30"
            />
          </div>
          {viewMode === 'my_products' ? (
            <select
              ...
              className="bg-dark-800 border border-accent-primary/12 rounded-lg px-3 py-2.5 text-sm text-text-secondary focus:outline-none focus:border-accent-primary/30"
            >
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            {(catBrand || catSeller || catMinPrice || catMaxPrice || catMinRating || catSponsored) && (
              <button
                ...
                className="text-xs text-text-muted hover:text-red-600 px-2 py-2 transition-colors whitespace-nowrap"
              >
                Clear Filters
              </button>
            )}
