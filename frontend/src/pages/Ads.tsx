...
export default function Ads() {
  ...
  return (
    <div className="space-y-5 md:space-y-6 animate-fade-in">
      <div>
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-text-primary">Ads</h1>
          <p className="text-sm md:text-base text-text-muted mt-1">Analyze sponsored products and brand advertisements</p>
        </div>
      </div>
        ...
        <div className="p-4 md:p-5">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-text-muted">Please select a search keyword</p>
            </div>
          ) : activeTab === 'sponsored' ? (
  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-text-muted">No sponsored products found for this search</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
      {products.map((product, index) => (
        <div key={index} className="card-dark border border-accent-primary/8 overflow-hidden hover:border-accent-primary/30 transition-all group">
          ...
          <div className="p-4">
            <h3 className="font-medium text-sm text-text-secondary line-clamp-2 mb-2 min-h-[2.5rem]">
  ...
  return (
    <div className="space-y-3">
      {brands.map((brand, index) => (
        <div key={index} className="rounded-lg border border-accent-primary/8 overflow-hidden">
          <button
            onClick={() => setExpandedBrand(expandedBrand === brand.seller_name ? null : brand.seller_name)}
            className="w-full px-4 py-4 flex items-start sm:items-center justify-between gap-2 bg-dark-800/50 hover:bg-surface-hover/50 transition-colors"
          >
            ...
            <svg 
              className={`w-5 h-5 text-text-muted transition-transform ${expandedBrand === brand.seller_name ? 'rotate-180' : ''}`}
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
          ...
          {expandedBrand === brand.seller_name && brand.products && brand.products.length > 0 && (
            <div className="p-3 md:p-4 border-t border-accent-primary/8 bg-dark-800/50">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {brand.products.map((product, pIndex) => (
                  <div key={pIndex} className="rounded-lg border border-accent-primary/8 overflow-hidden hover:border-accent-primary/20 transition-all bg-dark-800/30">
                    ...
                    <div className="p-3">
                      <p className="text-xs font-medium text-text-body line-clamp-2 mb-2 min-h-[2rem]">
