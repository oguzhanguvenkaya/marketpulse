...
export default function Products() {
  ...
  return (
    <div className="space-y-5 md:space-y-6 animate-fade-in">
      <div className="mb-2">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-[#0f1419] dark:text-[#F0FDF4]">Products</h1>
          <p className="text-sm md:text-base text-[#9e8b66] dark:text-[#6B8F80] mt-1">Browse and analyze collected product data</p>
        </div>
      </div>
      ...
      <div className="card-dark p-4 md:p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-[#0f1419] dark:text-[#F0FDF4]">Filter Products</h2>
        </div>
      ...
      {loading ? (
        <div className="card-dark p-10 md:p-12 text-center">
          ...
          <p className="mt-4 text-[#9e8b66] dark:text-[#6B8F80]">Loading products...</p>
        </div>
      ) : products.length === 0 ? (
        <div className="card-dark p-10 md:p-12 text-center">
          ...
          <p className="text-[#9e8b66] dark:text-[#6B8F80]">No products found yet</p>
                ...
                <tr key={product.id}>
                  <td>
                    <Link to={`/products/${product.id}`} className="flex items-center gap-3 group">
                      {product.image_url ? (
                        <img src={product.image_url} alt="" className="w-12 h-12 object-cover rounded-lg border border-[#5b4824]/12 dark:border-[#4ADE80]/12" />
                      ) : (
                        <div className="w-12 h-12 rounded-lg bg-[#f0e8d8] dark:bg-[#1C2E28] flex items-center justify-center">
                      ...
                      <div className="min-w-0">
                        <div className="font-medium text-sm text-[#3d3427] dark:text-[#A7C4B8] group-hover:text-accent-primary transition-colors line-clamp-2">
                          {product.name}
                        </div>
                        {product.seller_name && (
                  ...
                  <td>
                    {product.latest_rating ? (
                      <span className="flex items-center gap-1.5">
                        ...
                        <span className="text-[#3d3427] dark:text-[#A7C4B8]">{product.latest_rating.toFixed(1)}</span>
                      </span>
                    ) : (
                      <span className="text-neutral-500">-</span>
                    )}
                  </td>
                  <td className="text-[#9e8b66] dark:text-[#6B8F80]">
