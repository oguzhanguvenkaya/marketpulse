    ...
    <div className="space-y-5 md:space-y-6 animate-fade-in">
      <div className="mb-2 md:mb-4">
        <div>
          ...
          <p className="text-sm md:text-base text-text-muted mt-1">Monitor marketplace data and analytics</p>
        </div>
      </div>
      ...
      <div className="card-dark p-4 md:p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-text-primary">Keyword Search</h2>
        </div>
          ...
          <div className="mt-4 p-3 md:p-4 rounded-lg bg-accent-primary/5 border border-accent-primary/20">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div className="flex items-start md:items-center gap-3">
                ...
                <span className="text-sm md:text-base text-text-primary">Searching for "<span className="text-accent-primary">{currentTask.keyword}</span>"</span>
              </div>
              <div className="flex flex-wrap items-center gap-2 md:gap-3">
                {getStatusBadge(currentTask.status)}
                {currentTask.status === 'completed' && (
                  <span className="text-xs md:text-sm text-text-muted">{currentTask.total_products} products found</span>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="card-dark overflow-hidden">
        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-accent-primary/[0.08] flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-accent-primary/10 flex items-center justify-center">
            <svg className="w-4 h-4 text-accent-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="divide-y divide-accent-primary/[0.08]">
          {tasks.length === 0 ? (
            ...
            tasks.map((task, index) => (
              <div 
                key={task.id} 
                className="px-4 md:px-6 py-3 md:py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:bg-accent-primary/[0.03] transition-colors"
                style={{ animationDelay: `${index * 50}ms` }}
              >
                <div className="flex items-center gap-4">
                  <div className="w-9 h-9 md:w-10 md:h-10 rounded-lg bg-dark-800 dark:bg-dark-700 flex items-center justify-center">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
                  <span className="text-xs md:text-sm text-text-muted">{task.total_products} products</span>
