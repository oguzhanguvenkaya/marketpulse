  ...
  return (
    <div className="card-dark p-4 md:p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-lg font-semibold text-[#0f1419] dark:text-[#F0FDF4]">
      ...
      {!selectedProduct ? (
        <div className="text-center py-12">
          ...
          <p className="text-[#9e8b66] dark:text-[#6B8F80]">Select a product to view sellers</p>
        </div>
      ) : sellersLoading ? (
        <div className="text-center py-12">
        ...
        <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
          {sellers.map((seller, idx) => {
            ...
            <div
              ...
              className={`p-3 md:p-4 rounded-lg border transition-all cursor-pointer hover:bg-[#f0e8d8]/35 dark:hover:bg-[#1C2E28]/35 ${
                seller.price_alert
                  ? 'bg-danger/20 border-danger/35'
                  : seller.buybox_order === 1
                    ? 'bg-success/15 border-success/30'
                    : 'bg-[#f7eede]/45 dark:bg-[#162420]/45 border-[#5b4824]/12 dark:border-[#4ADE80]/12'
              }`}
            >
              <div className="flex items-start gap-3">
                {seller.merchant_logo && (
                  <img
                    ...
                    className="w-9 h-9 md:w-10 md:h-10 rounded-lg object-contain bg-white dark:bg-[#162420] border border-[#5b4824]/12 dark:border-[#4ADE80]/12"
                  />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                    <div>
                      <div className="font-medium text-sm flex items-center gap-2 flex-wrap">
                        {seller.merchant_url ? (
                          <a
                            ...
                            className="text-[#0f1419] dark:text-[#F0FDF4] hover:text-accent-primary transition-colors font-semibold"
                          >
                            {seller.merchant_name}
                          </a>
                        ) : (
                          <span className="text-[#0f1419] dark:text-[#F0FDF4] font-semibold">{seller.merchant_name}</span>
                        )}
                        {seller.buybox_order === 1 && (
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-[#5f471d] dark:text-[#A7C4B8]">
                    ...
                    <div className="text-left sm:text-right">
                      <div className="font-bold text-lg text-[#0f1419] dark:text-[#F0FDF4] flex items-center gap-2 justify-start sm:justify-end">
