    ...
    <div className="card-dark p-4 md:p-5">
      ...
      {!selectedProduct ? (
        <div className="text-center py-12">
          ...
          <p className="text-text-muted">Select a product to view sellers</p>
        </div>
      ) : sellersLoading ? (
        <div className="text-center py-12">
          <div className="w-8 h-8 border-2 border-accent-primary/30 border-t-accent-primary rounded-full animate-spin mx-auto" />
          <p className="text-neutral-500 dark:text-text-muted mt-3">Loading sellers...</p>
        </div>
      ) : sellers.length === 0 ? (
        <div className="text-center py-12">
          <div className="w-12 h-12 rounded-full bg-surface-hover flex items-center justify-center mx-auto mb-4">
          ...
          <p className="text-text-muted">No seller data yet</p>
          <p className="text-sm text-neutral-500 dark:text-text-muted mt-1">Click "Refresh" to fetch prices</p>
        </div>
      ) : (
        <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
          {sellers.map((seller, idx) => {
            ...
            <div
              ...
              className={`p-3 md:p-4 rounded-lg border transition-all cursor-pointer hover:bg-surface-hover/35 ${
                seller.price_alert
                  ? 'bg-danger/20 border-danger/35'
                  : seller.buybox_order === 1
                    ? 'bg-success/15 border-success/30'
                    : 'bg-dark-800/45 border-accent-primary/12'
              }`}
            >
              <div className="flex items-start gap-3">
                {seller.merchant_logo && (
                  <img
                    ...
                    className="w-9 h-9 md:w-10 md:h-10 rounded-lg object-contain bg-white dark:bg-surface-card border border-accent-primary/12"
                  />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                    <div>
                      <div className="font-medium text-sm flex items-center gap-2 flex-wrap">
                        {seller.merchant_url ? (
                          <a
                            ...
                            className="text-text-primary hover:text-accent-primary transition-colors font-semibold"
                          >
                            {seller.merchant_name}
                          </a>
                        ) : (
                          <span className="text-text-primary font-semibold">{seller.merchant_name}</span>
                        )}
                        {seller.buybox_order === 1 && (
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-text-body">
                    ...
                    <div className="text-left sm:text-right">
                      <div className="font-bold text-lg text-text-primary flex items-center gap-2 justify-start sm:justify-end">
