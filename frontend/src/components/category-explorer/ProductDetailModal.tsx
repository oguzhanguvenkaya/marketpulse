...
function ProductDetailPanel({ product, onClose, formatPrice, selectCategory }: {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-lg z-[9999] overflow-y-auto border-l border-accent-primary/12 shadow-2xl bg-gradient-to-b from-surface-card to-dark-900">
        <div className="sticky top-0 z-10 flex items-center justify-between p-4 border-b border-accent-primary/12 bg-surface-card/98 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-text-primary truncate">Product Details</h3>
          </div>
          <div className="flex items-center gap-2">
            <a href={product.source_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg hover:bg-accent-primary/8 text-text-muted">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-accent-primary/8 text-text-muted">
        ...
        <div className="p-4 space-y-4">
          {product.image_url && (
            <div className="rounded-lg bg-accent-primary/5 p-4 flex items-center justify-center">
              <img src={product.image_url} alt="" className="max-h-64 object-contain" />
            </div>
          )}
          <div>
            {product.brand && <div className="text-xs text-accent-primary font-medium uppercase mb-1">{product.brand}</div>}
            <h4 className="text-base font-medium text-text-primary leading-snug">{product.product_name}</h4>
          </div>
          {product.category && (
            <div>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {product.category.split(' > ').map((part, i, arr) => (
                  <span key={i} className="flex items-center gap-1">
                    {i > 0 && <svg className="w-2.5 h-2.5 text-text-faded" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>}
                    <button onClick={() => selectCategory(arr.slice(0, i + 1).join(' > '))} className="text-xs text-text-muted hover:text-accent-primary transition-colors">{part.trim()}</button>
                  </span>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {product.reviews.slice(0, 5).map((r, i) => (
                  <div key={i} className="rounded-lg bg-accent-primary/5 p-2 text-xs">
                    ...
                    <p className="text-text-body line-clamp-3">{r.text}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-lg z-[9999] overflow-y-auto border-l border-accent-primary/12 shadow-2xl bg-gradient-to-b from-surface-card to-dark-900">
        <div className="sticky top-0 z-10 flex items-center justify-between p-4 border-b border-accent-primary/12 bg-surface-card/98 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <span className="text-xs px-1.5 py-0.5 rounded font-medium bg-accent-primary/8 text-text-body font-mono">#{product.position}</span>
            <h3 className="text-base font-semibold text-text-primary truncate">Category Product</h3>
          ...
          <div className="flex items-center gap-2">
            {product.url && (
              <a href={product.url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg hover:bg-accent-primary/8 text-text-muted">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
              </a>
            )}
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            {product.brand && <div className="text-xs text-accent-primary font-medium uppercase mb-1">{product.brand}</div>}
            <h4 className="text-base font-medium text-text-primary leading-snug">{product.name}</h4>
          </div>
              ...
              <div className="space-y-1">
                {Object.entries(product.specs).map(([k, v]) => (
                  <div key={k} className="flex justify-between text-xs py-1 px-2 rounded bg-accent-primary/5">
                    ...
                    <span className="text-text-primary text-right max-w-[60%]">{String(v)}</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    <>
      ...
      {showMobileFilters && createPortal(
        <>
          ...
          <div className="fixed top-0 left-0 h-full w-80 z-[9997] overflow-y-auto border-r border-accent-primary/12 shadow-2xl p-4 bg-gradient-to-b from-surface-card to-dark-900">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-text-primary">Filters</h3>
              <button onClick={() => setShowMobileFilters(false)} className="p-1.5 rounded-lg hover:bg-accent-primary/8 text-text-muted">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <CategoryFilters {...ce} />
