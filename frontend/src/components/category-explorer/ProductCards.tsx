...
function StoreProductCard({
  ...
  return (
    <div
      key={product.id}
      className="rounded-xl border border-[#5b4824]/12 dark:border-[#4ADE80]/12 overflow-hidden hover:border-[#5b4824]/15 dark:hover:border-[#4ADE80]/15 transition-all cursor-pointer group bg-gradient-to-b from-[#fefbf0] to-[#fffbef] dark:from-[#162420] dark:to-[#0F1A17]"
      onClick={() => { setSelectedProduct(product); setSelectedCatProduct(null); }}
    >
      <div className="flex gap-3 p-3">
        <div className="w-20 h-20 rounded-lg bg-[#5b4824]/5 dark:bg-[#4ADE80]/5 flex items-center justify-center flex-shrink-0 overflow-hidden">
          {product.image_url ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            {product.brand && <span className="text-[10px] text-[#5b4824] dark:text-[#4ADE80] font-medium uppercase truncate">{product.brand}</span>}
          </div>
          <h3 className="text-sm text-[#3d3427] dark:text-[#A7C4B8] line-clamp-2 leading-snug mb-1.5">{product.product_name || 'Unnamed'}</h3>
          <div className="flex items-end justify-between">
            <span className="text-base font-bold text-[#0f1419] dark:text-[#F0FDF4]">{formatPrice(product.price)}</span>
            {product.rating && (
              <div className="flex items-center gap-0.5">
                ...
                <span className="text-xs text-[#9e8b66] dark:text-[#6B8F80]">{product.rating}</span>
                {product.review_count != null && <span className="text-[10px] text-[#b5a382] dark:text-[#6B8F80]">({product.review_count})</span>}
              </div>
            )}
          </div>
        </div>
      ...
      {product.category && (
        <div className="px-3 pb-2.5">
          <p className="text-[10px] text-[#b5a382] dark:text-[#6B8F80] truncate">{product.category}</p>
        </div>
      )}
    </div>
  );
...
function CatProductCard({
  ...
  return (
    <div
      key={product.id}
      className={`rounded-xl border overflow-hidden transition-all cursor-pointer group relative bg-gradient-to-b from-[#fefbf0] to-[#fffbef] dark:from-[#162420] dark:to-[#0F1A17] ${
        isSelected ? 'border-[#5b4824]/25 dark:border-[#4ADE80]/25 ring-1 ring-[#5b4824]/15 dark:ring-[#4ADE80]/15' : 'border-[#5b4824]/12 dark:border-[#4ADE80]/12 hover:border-[#5b4824]/15 dark:hover:border-[#4ADE80]/15'
      }`}
    >
      <div className="absolute top-2 right-2 flex items-center gap-1 z-10">
        <button
          ...
          className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
            isSelected ? 'bg-[#5b4824] dark:bg-[#4ADE80] border-[#5b4824] dark:border-[#4ADE80] text-[#0f1419] dark:text-[#0F1A17]' : 'border-[#5b4824]/15 dark:border-[#4ADE80]/15 hover:border-[#5b4824]/30 dark:hover:border-[#4ADE80]/30'
          }`}
        >
          {isSelected && (
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#5b4824]/8 dark:bg-[#4ADE80]/8 text-[#9e8b66] dark:text-[#6B8F80] font-mono">
          #{product.position}
        </span>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="text-sm text-[#3d3427] dark:text-[#A7C4B8] line-clamp-2 leading-snug mb-1.5">{product.name || 'Unnamed'}</h3>
          <div className="flex items-end justify-between">
      ...
      <div className="px-3 pb-2.5 flex items-center justify-between">
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-[10px] text-[#b5a382] dark:text-[#6B8F80]">Page {product.page_number}</span>
          {product.seller_name && <span className="text-[10px] text-[#b5a382] dark:text-[#6B8F80] truncate">| {product.seller_name}</span>}
          {product.sku && <span className="text-[10px] text-[#b5a382] dark:text-[#6B8F80] font-mono truncate">SKU: {product.sku}</span>}
        </div>
    ...
    return (
      <div className="flex items-center justify-center gap-2 pt-4">
        <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
          className="px-3 py-2 text-sm rounded-lg border border-[#5b4824]/12 dark:border-[#4ADE80]/12 text-[#5f471d] dark:text-[#A7C4B8] hover:bg-[#5b4824]/5 dark:hover:bg-[#4ADE80]/5 disabled:opacity-30">Previous</button>
          ...
          {Array.from({ length: Math.min(5, currentTotalPages) }, (_, i) => {
            ...
            return (
              <button key={p} onClick={() => setPage(p)}
                className={`w-9 h-9 text-sm rounded-lg ${p === page ? 'bg-[#5b4824]/10 dark:bg-[#4ADE80]/10 text-[#5b4824] dark:text-[#4ADE80] border border-[#5b4824]/20 dark:border-[#4ADE80]/20' : 'text-[#9e8b66] dark:text-[#6B8F80] hover:bg-[#5b4824]/5 dark:hover:bg-[#4ADE80]/5'}`}
              >{p}</button>
            );
          })}
        </div>
        <button onClick={() => setPage(p => Math.min(currentTotalPages, p + 1))} disabled={page >= currentTotalPages}
          className="px-3 py-2 text-sm rounded-lg border border-[#5b4824]/12 dark:border-[#4ADE80]/12 text-[#5f471d] dark:text-[#A7C4B8] hover:bg-[#5b4824]/5 dark:hover:bg-[#4ADE80]/5 disabled:opacity-30">Next</button>
        <span className="text-xs text-[#b5a382] dark:text-[#6B8F80] ml-2">{currentTotal.toLocaleString()} products</span>
      </div>
    );
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-8 h-8 border-2 border-[#5b4824] dark:border-[#4ADE80] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
      ...
      <>
        <div className="flex items-center justify-between mb-3 px-1">
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 text-xs rounded-lg border transition-colors ${
                selectedForDetail.size === (catData?.products?.length || 0) && selectedForDetail.size > 0
                  ? 'border-[#5b4824]/25 dark:border-[#4ADE80]/25 bg-[#5b4824]/8 dark:bg-[#4ADE80]/8 text-[#5b4824] dark:text-[#4ADE80]'
                  : 'border-[#5b4824]/12 dark:border-[#4ADE80]/12 text-[#5f471d] dark:text-[#A7C4B8] hover:bg-[#5b4824]/8 dark:hover:bg-[#4ADE80]/8'
              }`}
            >
              <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                selectedForDetail.size === (catData?.products?.length || 0) && selectedForDetail.size > 0
                  ? 'bg-[#5b4824] dark:bg-[#4ADE80] border-[#5b4824] dark:border-[#4ADE80]'
                  : selectedForDetail.size > 0
                  ? 'bg-cyan-500/50 border-[#5b4824]/30 dark:border-[#4ADE80]/30'
                  : 'border-[#5b4824]/20 dark:border-[#4ADE80]/20'
              }`}>
                {selectedForDetail.size > 0 && (
                  <svg className="w-3 h-3 text-[#0f1419] dark:text-[#0F1A17]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {selectedForDetail.size > 0 && (
              <span className="text-xs text-[#5b4824] dark:text-[#4ADE80]">
...
function EmptyState({ viewMode, hasFilters }: { viewMode: string; hasFilters: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="w-16 h-16 rounded-2xl bg-[#5b4824]/5 dark:bg-[#4ADE80]/5 flex items-center justify-center mb-4">
      ...
      <h3 className="text-lg font-medium text-[#5f471d] dark:text-[#A7C4B8] mb-2">
