    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 space-y-3">
          ...
          <p className="text-neutral-500 text-xs">Click "Scrape Products" to fetch products from Price Monitor, or adjust filters.</p>
        </div>
      )}
      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setSelectedProduct(null)}>
          <div
            className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-white/10 p-6 space-y-4"
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1 pr-4">
                ...
                <h2 className="text-lg font-bold text-white">{selectedProduct.product_name}</h2>
              </div>
              <button onClick={() => setSelectedProduct(null)} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-neutral-400">
              ...
              <div>
                {selectedProduct.image_url && (
                  <div className="rounded-xl overflow-hidden bg-black/30 border border-white/5">
                    <img src={selectedProduct.image_url} alt="" className="w-full object-contain max-h-80" />
                  </div>
                )}
                {selectedProduct.images && selectedProduct.images.length > 1 && (
                  <div className="flex gap-2 mt-2 overflow-x-auto pb-2">
                    {selectedProduct.images.slice(0, 6).map((img, i) => (
                      <img key={i} src={img} alt="" className="w-16 h-16 object-contain rounded-lg border border-white/10 bg-black/20 flex-shrink-0" />
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg p-3 bg-white/[0.03] border border-white/[0.06]">
                    ...
                    <p className="text-lg font-bold text-white">{formatPrice(selectedProduct.price, selectedProduct.currency)}</p>
                  </div>
                  <div className="rounded-lg p-3 bg-white/[0.03] border border-white/[0.06]">
                ...
                <div className="space-y-2 text-sm">
                  {selectedProduct.sku && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-neutral-200 font-mono text-xs">{selectedProduct.sku}</span>
                    </div>
                  )}
                  {selectedProduct.barcode && (
                    <div className="flex justify-between">
                      <span className="text-neutral-500">Barcode</span>
                      <span className="text-neutral-200 font-mono text-xs">{selectedProduct.barcode}</span>
                    </div>
                  )}
                  {selectedProduct.seller_name && (
                    <div className="flex justify-between">
                      <span className="text-neutral-500">Seller</span>
                      <span className="text-neutral-200">{selectedProduct.seller_name}</span>
                    </div>
                  )}
                ...
                {selectedProduct.category_breadcrumbs && (
                  <div>
                    <p className="text-[10px] text-neutral-500 uppercase tracking-wider mb-1">Category</p>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-1">
                  {Object.entries(selectedProduct.product_specs).map(([key, val]) => (
                    <div key={key} className="flex gap-2 text-xs py-1 px-2 rounded bg-white/[0.02]">
                      <span className="text-neutral-500 min-w-[100px]">{key}</span>
