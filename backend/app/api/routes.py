...
@router.get("/test-trendyol-scraper")
async def test_trendyol_scraper(
    sku: str = Query(..., description="Trendyol SKU to test"),
    mode: str = Query("basic", description="ScraperAPI mode: basic, render, premium, ultra_premium")
):
    """Test different ScraperAPI configurations for Trendyol"""
    import aiohttp
    import ssl
    import random
    import os
    
    api_key = os.environ.get('SCRAPPER_API', '')
    if not api_key:
        try:
            with open('/etc/secrets/SCRAPPER_API', 'r') as f:
                api_key = f.read().strip()
        except:
            return {"error": "SCRAPPER_API not found"}
    
    url = f"https://www.trendyol.com/x/x-p-{sku}"
    ...
    modes_config = {
        "basic": f"scraperapi.session_number={session_num}",
        "basic_tr": f"scraperapi.country_code=tr.session_number={session_num}",
        "render": f"scraperapi.render=true.session_number={session_num}",
        "render_tr": f"scraperapi.render=true.country_code=tr.session_number={session_num}",
        "premium": f"scraperapi.premium=true.session_number={session_num}",
        "premium_tr": f"scraperapi.premium=true.country_code=tr.session_number={session_num}",
        "ultra_premium": f"scraperapi.ultra_premium=true.session_number={session_num}",
        "render_premium": f"scraperapi.render=true.premium=true.session_number={session_num}",
        "render_ultra": f"scraperapi.render=true.ultra_premium=true.session_number={session_num}"
    }
    
    if mode == "all":
        test_modes = list(modes_config.keys())
    else:
        test_modes = [mode] if mode in modes_config else ["basic"]
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        for test_mode in test_modes:
            proxy_user = modes_config[test_mode]
            proxy_auth = aiohttp.BasicAuth(proxy_user, api_key)
            ...
            try:
                async with session.get(
                    ...
                    has_slider = 'slider__slide' in html or 'other-merchant' in html
                    has_price = 'price' in html.lower()
                    
                    results[test_mode] = {
                        "status": resp.status,
                        "success": resp.status == 200,
                        "has_seller_slider": has_slider,
                        "has_price_data": has_price,
                        "html_length": len(html),
                        "sample": html[:500] if resp.status == 200 else html[:200]
                    }
            except Exception as e:
                results[test_mode] = {
                    "status": 0,
